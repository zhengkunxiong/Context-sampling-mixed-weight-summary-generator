...
from transformers import RagConfig, AutoModel
...
class DynamicRagModel(DynamicRagPreTrainedModel):
    def __init__(
        ...
        self.use_overall_model=config.use_overall_model
        if config.use_overall_model:
            self.nhead=16
            self.encoder_layer = nn.TransformerEncoderLayer(d_model=config.d_model, nhead=self.nhead)
            self.overall_encoder=torch.nn.TransformerEncoder(self.encoder_layer, num_layers=2, norm=None)
    ...
    def forward(
        ...
        decoder_features = gen_outputs.decoder_hidden_states[-1].contiguous().view(B, self.config.n_docs, -1, self.config.d_model)  # shape = (B, n_doc, T, H_g) 1 20 200 64
        dynamic_scores = self.dynamic_mlp(decoder_features).squeeze(3)  # shape = (B, n_docs, T)
        if self.use_overall_model:
            if overall_scores == None:
                encoder_last=gen_outputs.encoder_last_hidden_state
                Sentence_repre=encoder_last[:,0,:].contiguous().view(B,-1,self.config.d_model)
                output_overall_encoder=self.overall_encoder(Sentence_repre)
                overall_scores=self.overall_mlp(output_overall_encoder.squeeze(0)).unsqueeze(0)
                dynamic_scores=dynamic_scores+overall_scores
...
class DynamicRagForGeneration(DynamicRagPreTrainedModel):
    ...
    @classmethod
    def from_pretrained(cls, pretrained_model_name_or_path, n_docs, gradient_checkpointing,use_overall_model,max_source_len=None):
        # logging.debug(pretrained_model_name_or_path)
        if 'bart' in pretrained_model_name_or_path:
            from transformers import BartForConditionalGeneration
            generator = BartForConditionalGeneration.from_pretrained(pretrained_model_name_or_path,cache_dir=r"models/models--facebook--bart-large",local_files_only=1)
        ...
        config.max_source_len=max_source_len
        config.gradient_checkpointing = gradient_checkpointing
        config.do_marginalize = False
        config.reduce_loss = False
        config.label_smoothing = 0.0
        config.use_overall_model=use_overall_model
    ...
    def generate(
        ...
        if self.rag.config.use_overall_model:
            overall_encoder=self.rag.overall_encoder
            overall_mlp=self.rag.overall_mlp
            encoder_last = last_hidden_state
            encoder_last = encoder_last[:,0,:].contiguous().view(batch_size, -1, self.config.d_model)
            output_overall_encoder = overall_encoder(encoder_last)
            overall_scores = overall_mlp(output_overall_encoder.squeeze(0)).unsqueeze(0).repeat_interleave(num_beams, dim=0)
            model_kwargs["overall_scores"]=overall_scores
