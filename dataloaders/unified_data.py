...
class DatasetBase(data.Dataset):
    ...
    def get_contextualized_turn_next(self,text, turn_id, i,sentences_num):
        get_id = turn_id + i
        i = -i
        if i > 0:
            i += 1
        if 0 <= get_id and get_id < sentences_num:
            turn_next = text[get_id]
            return turn_next, i
        else:
            return None, i

    def get_word_num(self,text1,text2=''):
        if text2==None:
            text2=''
        return len(text1.split())+len(text2.split())

    def dy_window_size(self,max_len,padded_text,turn_id):
        bos_token=self.generator_tokenizer.bos_token+" "
        eos_token=" "+self.generator_tokenizer.eos_token+" "
        sentences_num=len(padded_text)
        contextualized_turn=padded_text[turn_id]
        i=1
        turn_next, i = self.get_contextualized_turn_next(padded_text, turn_id, i, sentences_num)
        while self.get_word_num(contextualized_turn, turn_next)<=max_len:
            if turn_next!=None:
                if i<0:
                    contextualized_turn=contextualized_turn+eos_token+turn_next
                else:
                    contextualized_turn=turn_next+eos_token+contextualized_turn

            # else:
            #     print(self.get_word_num(contextualized_turn))
            turn_next, i = self.get_contextualized_turn_next(padded_text, turn_id, i, sentences_num)
        contextualized_turn = bos_token+contextualized_turn
        return contextualized_turn
    ...
    def tokenize_generator(self, text, query, summary):
        ...
        for turn_id in range(len(text)):
            ...
            contextualized_turn=self.dy_window_size(config.max_source_len,padded_text,turn_id)
            contextualized_turn=query+ " // " + contextualized_turn
            input_dict = self.generator_tokenizer.prepare_seq2seq_batch(src_texts=contextualized_turn ,
                                                                        tgt_texts=summary,
