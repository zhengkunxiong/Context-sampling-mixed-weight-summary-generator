...
from transformers import (AutoTokenizer, AutoModel,RobertaTokenizer, RobertaForTokenClassification,
                          BartTokenizer,BertForTokenClassification,BertTokenizer,BertForPreTraining,
                          AdamW)
...
class Experiment(object):
    ...
    def train_epoch(self, epoch_id):
        ...
        if config.debug:
            ...
            for i in train_dataloader:
                c+=1
                loader.append(i)
                if c==config.save_steps*config.gradient_accumulation_steps:
                    break
        ...
        for data in tqdm(train_dataloader):
            ...
            if config.target_task in ['qmsum-latent',
                ...
                # Forward.
                input=retriever_input_ids.squeeze(0)
                retriever_outputs = self.retriever(input_ids=input,
                ...
                # input=oracle[0]
                # ret_loss =self.criterion_cls(input=retriever_cls_logits,target=gpu_wrapper()))
                ...
                if self.iter_num % config.save_steps*config.gradient_accumulation_steps == 0:
                    print(float(total_loss/config.save_steps))
                    total_loss=0
    ...
    def seq_evaluate_gen(self, test, beam_size):
        ...
        eval_dataloader = DataLoader(the_set, batch_size=1, shuffle=False, num_workers=config.num_workers,pin_memory=True)
        ...
        filename = '{}rouge_scores.txt'.format('test' if test else 'valid')
        ...
        if not test and metric > self.best_metric:
            ...
            if not config.debug:
                ...
                with open(filename, 'a') as file_object:
                    file_object.write("get_best_model"+datetime.datetime.now().strftime('%Y-%m-%d  %H:%M:%S') + "\n")
            peep_num = 3
        ...
        with open(filename, 'a') as file_object:
            file_object.write(str(rouge1)+str(rouge2)+str(rougeL)+datetime.datetime.now().strftime('%Y-%m-%d  %H:%M:%S') +"\n")
